...
const confirmRemoveInterview = async (id: string): Promise<void> => {
  confirm.require({
    ...
    header: 'Удаление собседования',
    icon: 'pi pi-info-circle',
    rejectLabel: 'Отмена',
    acceptLabel: 'Удалить',
    rejectClass: 'p-button-secendory p-button-outlined',
    acceptClass: 'p-button-danger',
    accept: async () => {
      isLoading.value = true
      await deleteDoc(doc(db, `users/${userStore.userId}/interviews`, id))
      await getListInterwews()
      isLoading.value = false
    },
  })
}

const getListInterwews = async (isFilter?: boolean): Promise<void> => {
...
<template>
  <app-dialog />
  <app-progress v-if="isLoading" class="spiner" />
